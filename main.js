class Sky {
  ...
  updateStars() {
    this.stars.forEach((star) => {
      ...
      star.y -=
        (star.speed * (this.delta / 8) * (this.width / 2 - star.x)) / 3000;
  ...
  updateConstellation() {
    if (this.constellation.width > 0) {
      this.constellation.width -= 0.03 * (this.delta / 8);
  ...
  draw(now) {
    ...
    this.lastUpdate = now;
    window.requestAnimationFrame((now) => this.draw(now)); //zamiar wykonania animacji
  }
